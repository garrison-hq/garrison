...
**Last updated**: 2026-06-12 (M11 amendment — `request_external_action` verb as the 12th chat verb, pre-implementation per Rule 1 / M11 spec FR-001 / FR-030; three new Server-Action verbs `approve_action`, `reject_action`, `mark_action_done`). Previous: 2026-06-11 (M9 amendment). Initial: 2026-04-29 (M5.3 amendment establishes the chat mutation surface).
...
**M9** — second amendment (landed before any M9 verb code per Rule 1 and M9 spec FR-601; scope per `specs/_context/m9-context.md` §Scope extensions). Adds `create_scheduled_task` as the **eleventh** chat verb (Tier 3 — see §5) and records the four M9 dashboard-only scheduled-task verbs in the Server-Action registry note (§5, "Server-Action verb registry"). The chat verb count moves **10 → 11**: the ninth and tenth verbs (`propose_skill_change`, `bump_skill_version`) landed in M7 with their threat treatment in the sibling `docs/security/hiring-threat-model.md`; this amendment backfills their tier entries into §5 so the table enumerates the full sealed registry.

**M11** — third amendment (this update, landed before any `internal/actionbroker/` code per Rule 1, M11 spec FR-001 / FR-030, and the `action-broker-threat-model.md` threat-model-first discipline). Adds `request_external_action` as the **twelfth** chat verb (Tier 3 — see §5 below). The chat verb count moves **11 → 12**. Also adds three new Server-Action verbs (`approve_action`, `reject_action`, `mark_action_done`) to the Server-Action registry note, and records the `agentVerbNames()` widening in `tool.go` as a Rule-1 amendment (agent-caller surface widens to include `request_external_action` — the first verb callable by agents but not by the chat-CEO, per Q-D resolution in `specs/023-m11-action-broker/spec.md`).
...
The twelve chat verbs (eight from M5.3, two from M7, one from M9, one from M11) are classified as follows. The classification is binding. The two M7 verbs' full threat treatment lives in the sibling `docs/security/hiring-threat-model.md`; they are enumerated here so this table matches the sealed `Verbs` registry one-for-one.
...
- **`request_external_action(action_type, target, payload, ticket_id?)`** (M11): writes one immutable `work.pending_actions` row describing a requested outbound external action (e.g. a GitHub issue comment) and does **not** act on the world. Tier 3 rationale: **queues an attacker-influenceable effect on the outside world; an executed external action does not reverse** (a posted GitHub comment cannot be unposted without a separate corrective action). The verb is **agent-callers only** — `AgentInstanceID` must be a valid non-zero UUID; a chat-CEO or non-agent caller is rejected with `validation_failed("request_external_action: callable only by agents")`. This is the inverse of M9's chat-only guard and the first verb in the registry callable by agents but not by the chat-CEO (Q-D resolution). The tier is assigned by the policy table at write time (never agent-supplied); `github_issue_comment` is on the permanent-Approve floor and cannot be classified below `approve`. The verb returns a typed result saying the action is *queued and at which tier* — never "done". Audit `args_jsonb` captures the full input args, anchored on `agent_instance_id` (M8 audit anchor). The `agentVerbNames()` function in `tool.go` includes this verb, widening the agent-caller surface by one (this is the Rule-1 amendment recorded in the M11 `action-broker-threat-model.md`). Full threat treatment is in `docs/security/action-broker-threat-model.md`; this entry records the reversibility classification and the agent-caller-only posture for the sealed registry.
...
M11 adds three action-broker entries; their audit rows are written dashboard-side in the same drizzle transaction as the status transition, with both chat anchors NULL and `affected_resource_type='pending_action'`:

- **`approve_action`** — class 1. Transitions a `pending_actions` row from `pending` to `approved` (recording the approving operator in `approved_by`) and emits `work.action.dispatch_requested` for the dispatcher. Reversal: the operator can call `reject_action` before the dispatcher claims the row (race-dependent). Audit row records the `pending_action_id` and the approving operator.
- **`reject_action`** — class 1. Transitions a `pending_actions` row to `rejected`. Reversal: the operator can re-request the action via `request_external_action`. Audit row records the `pending_action_id`.
- **`mark_action_done`** — class 1. Transitions a `human_only` `pending_actions` row to `done`, recording an optional free-text note of what was actually performed. The dispatcher never executed the row; the operator performed the action by hand. Audit row records the `pending_action_id` and the optional note.

These three verbs are part of the registry/tier table and the `chat_mutation_audit` verb CHECK (M11 migration extends the `verb` CHECK to include `approve_action`, `reject_action`, `mark_action_done`; the `affected_resource_type` CHECK gains `pending_action`), not of the twelve-verb chat surface.
