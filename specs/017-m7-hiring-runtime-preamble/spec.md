...
- Q: Recovery from supervisor crash mid-install. → A: **Journal each install step in `chat_mutation_audit`** with `kind='install_step:<step>'` rows; supervisor restart resumes from the last journaled step or atomically rolls back. Steps: `download`, `verify_digest`, `extract`, `mount`, `container_create`, `container_start`. Each step's audit row carries its outcome (success / fail / interrupted). Matches the M2.2.1 finalize-as-journal precedent.
...
- **FR-214a**: System MUST journal every material install step in `chat_mutation_audit` with `kind='install_step:<step>'` (one row per: `download`, `verify_digest`, `extract`, `mount`, `container_create`, `container_start`). On supervisor restart with an `install_in_progress` proposal, the supervisor reads the most recent install_step row for that proposal: if the step succeeded, resume from the next step; if interrupted (no terminal outcome row), atomically roll back any side effects (delete partially-extracted skill dir, remove created-but-not-started container) and mark the proposal `install_failed: interrupted_by_supervisor_crash`, retryable via operator-button.
