...
### Session 2026-05-02 (post-spec /speckit.clarify)

- Q: Skill archive format the install actuator must decompress. → A: **tar.gz only** (`archive/tar` + `compress/gzip` stdlib, single happy path). If SkillHub turns out to serve a different format at the §8.5 operator-side spike, plan amends with a one-line justification.
- Q: Concurrent skill-bump conflict — two pending version-bump proposals for the same `(agent, skill)`. → A: **No DB-side dedup; both proposals valid; approving one auto-rejects siblings via supersession.** The auto-reject mechanism flips the sibling pending rows to `status='rejected'` with `rejected_reason='superseded_by:<approved.id>'` in the same Server Action transaction as the approval write. Preserves FR-102 immutability; matches the M5.3 sibling-mutations-land-independently pattern.
- Q: Recovery from supervisor crash mid-install. → A: **Journal each install step in a new `agent_install_journal` table** with one row per step `(download, verify_digest, extract, mount, container_create, container_start)`; supervisor restart resumes from the last journaled step or atomically rolls back. Each step's row carries its outcome (`success / failed / interrupted`). Matches the M2.2.1 finalize-as-journal precedent. **Schema deviation note** (T001 implementation, 2026-05-04): the original draft of this clarification placed the journal in `chat_mutation_audit`. The existing M5.3 table uses a `verb` column whose semantic surface is "operator-authorized chat mutation"; install-step rows are supervisor-driven audit events, not chat verbs. Forcing them into the existing table either bends `verb` to non-verb identifiers or adds a second-purpose column. The new dedicated table preserves FR-214a's intent (per-step audit + crash recovery) cleanly. See migration `20260504000000_m7_hiring_runtime_preamble.sql` for the table shape.
- Q: Skill registry version-string semantics. → A: **Any string the registry serves; digest pin is the integrity anchor.** No SemVer enforcement at propose-time. The approval UX surfaces both the version string AND the digest; the operator decides whether the version-bump is "newer." Compatible with skills.sh's arbitrary-tag publish convention and any future registry that doesn't enforce SemVer.
- Q: Per-agent UID allocator algorithm within the per-customer range. → A: **Sequential within range; `agents.host_uid` populated at activation as `MAX(host_uid)+1` over rows with `host_uid` in the customer's range.** Trivial single-statement allocation, operator-readable on host-side `ls -la`, persisted in the same transaction as the `agents` row INSERT (or the migration row for grandfathered M2.x agents).
...
- **FR-106a**: System MUST accept any non-empty version string the registry serves (no SemVer enforcement). The version string is recorded for operator readability; the digest is the integrity anchor. The approval UX MUST display BOTH the version string AND the digest side by side; the operator decides whether a version-bump represents a forward step.
- **FR-107**: System MUST decompress skill packages as **tar.gz only** (using `archive/tar` + `compress/gzip` stdlib; no new dependency). Non-tar.gz payloads are rejected at install time with `install_failed: unsupported_archive_format`. The install actuator MUST validate every archive entry's path during extract (no `..`, no absolute, no symlinks pointing outside) and reject the entire archive on any violation (HR-8). If SkillHub's §8.5 operator-side spike surfaces a different served format, plan amends FR-107 with one-line justification.
...
- **FR-110a**: System MUST auto-reject sibling pending proposals for the same `(target_agent_id, skill_package)` pair when one is approved. The supersession write happens in the same Server Action transaction as the approval; sibling rows transition to `status='rejected'` with `rejected_reason='superseded_by:<approved_audit_id>'`. Preserves FR-102 (proposal immutability). No DB-side unique index — two simultaneous `bump_skill_version` chat calls both succeed at propose-time; supersession resolves at approve-time.
...
- **FR-206a**: System MUST allocate `agents.host_uid` **sequentially within the per-customer range** at agent activation. The allocation query is `MAX(host_uid)+1` over rows whose `host_uid` falls in the activating agent's customer range, persisted in the same transaction as the `agents` INSERT (or the grandfathering migration row for M2.x agents). Range exhaustion (allocator returns a value above the range ceiling) errors the activation; operator widens the range in supervisor config.
...
- **FR-214a**: System MUST journal every material install step in the dedicated `agent_install_journal` table (one row per: `download`, `verify_digest`, `extract`, `mount`, `container_create`, `container_start`; columns `proposal_id`, `step`, `outcome`, `error_kind`, `payload_jsonb`, `created_at`). On supervisor restart with an `install_in_progress` proposal, the supervisor reads the most recent journal row for that proposal: if the step succeeded, resume from the next step; if interrupted (no terminal outcome row), atomically roll back any side effects (delete partially-extracted skill dir, remove created-but-not-started container) and mark the proposal `install_failed: interrupted_by_supervisor_crash`, retryable via operator-button. *(Original spec text placed this journal in `chat_mutation_audit`; T001's migration relocated it to a dedicated table because chat_mutation_audit's `verb` column carries chat-mutation semantics that don't fit supervisor-driven install events. See the Clarifications session entry above for the full deviation note.)*
...
- **chat_mutation_audit** (existing M5.3 table). M7 extends with `retention_class TEXT NULL` (FR-404). New `verb` values added to the existing CHECK constraint: `propose_skill_change`, `bump_skill_version`, `approve_hire`, `reject_hire`, `approve_skill_change`, `reject_skill_change`, `approve_version_bump`, `reject_version_bump`, `update_agent_md`, `grandfathered_at_m7`. Install-step events do NOT land here — see `agent_install_journal` below.
- **agent_install_journal** (new M7 table — see FR-214a). Per-step audit for the install pipeline; one row per step per proposal. Columns: `id`, `proposal_id` (FK), `step`, `outcome`, `error_kind`, `payload_jsonb`, `created_at`. Append-only.
