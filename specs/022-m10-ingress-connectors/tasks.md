...
- [x] **T008** `internal/ingress/server.go` + `internal/ingress/config.go` (vault fetch) + `cmd/supervisor/main.go` wiring
...
- [x] **T015** Dashboard surfaces — ingress-origin chip, ticket detail external link, connector-status page, `GET /ingress/status` endpoint
  - **Depends on**: T008.
...
- [x] **T018** Retro — `docs/retros/m10.md` + MemPalace drawer mirror
