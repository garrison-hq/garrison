...
- [ ] **T002** Goose migration `20260612000000_m10_ingress_connectors.sql` + sqlc query file + drizzle pull
  - **Depends on**: T001.
...
- [ ] **T004** `internal/ingress/connector.go` + `signature.go` — connector interface + HMAC verification
  - **Depends on**: T002.
...
- [ ] **T005** `internal/ingress/github.go` — GitHub connector Filter + Map + config struct
...
- [ ] **T006** `internal/ingress/idempotency.go` + `handler.go` (success path, no rate cap yet)
...
- [ ] **T007** `internal/ingress/ratecap.go` + `internal/throttle/ingress.go` — per-connector token bucket + M6 evidence
