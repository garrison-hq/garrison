...
- **FR-040**: A new `chat_sessions` table MUST be created with at minimum: `id` (UUID PK), `started_by_user_id` (UUID NOT NULL — semantically references `users.id` but **no cross-domain FK constraint**, matching the M4 retro's `metadata.actor_user_id` discipline of avoiding supervisor → dashboard FKs; integrity is preserved because the dashboard server action takes the value from an authenticated better-auth session), `started_at` (timestamptz), `ended_at` (timestamptz nullable), `status` (TEXT enum-checked: `active|ended|aborted`), `total_cost_usd` (NUMERIC default 0), `claude_session_label` (TEXT nullable — operator-facing label, optional in M5.1).
