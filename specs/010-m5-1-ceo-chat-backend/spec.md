...
- **FR-041**: A new `chat_messages` table MUST be created with at minimum: `id` (UUID PK), `session_id` (UUID FK to `chat_sessions.id`), `turn_index` (INTEGER), `role` (TEXT enum-checked: `operator|assistant`), `content` (TEXT — never NULL on terminal commit), `tokens_input` (INTEGER nullable), `tokens_output` (INTEGER nullable), `cost_usd` (NUMERIC nullable), `error_kind` (TEXT nullable, with documented vocabulary), `raw_event_envelope` (JSONB nullable), `created_at` (timestamptz), `terminated_at` (timestamptz nullable). Index on `(session_id, turn_index)`.
...
- **FR-050**: The dashboard MUST INSERT new operator messages into `chat_messages` (role='operator') and emit `pg_notify('chat.message.sent', message_id)`. The supervisor LISTENs on `chat.message.sent` and processes messages in the order received.
- **FR-051**: The supervisor MUST emit `pg_notify('chat.assistant.delta', message_id)` after every batch of token deltas appended to `chat_messages.raw_event_envelope` (batch size and frequency are spec-implementation-defined; first delta MUST land within 1s of the container's first text_delta event).
- **FR-052**: A new dashboard SSE endpoint at `/api/sse/chat?session_id=<uuid>` MUST stream the supervisor's deltas (read from `chat_messages.raw_event_envelope` in response to `chat.assistant.delta` notifies) plus the terminal `result` and `session_ended` events. Reconnect via `Last-Event-ID` MUST work the same way as the M3 activity feed.
...
- **FR-061**: A per-session soft cap configurable via `GARRISON_CHAT_SESSION_COST_CAP_USD` (default 1.00) MUST be enforced before spawn. If the projected next-turn cost (current `chat_sessions.total_cost_usd` + a small buffer, exact heuristic spec-implementation-defined) exceeds the cap, the supervisor MUST refuse the spawn and write an assistant row with `error_kind='session_cost_cap_reached'`.
...
- **FR-080**: A chat session MUST be created (status='active') on the first operator message of a session. Session creation is implicit — there is no separate "open session" call.
...
- **FR-083**: On supervisor boot, any session with `status='active'` whose newest `chat_messages` row has a `pending` operator turn older than 60s MUST be marked `aborted` and the pending message terminal-written with `error_kind='supervisor_restart'`.
...
- **SC-012**: Soft cost cap enforces: with `GARRISON_CHAT_SESSION_COST_CAP_USD=0.001` and a session whose running cost is already $0.0009, the next operator message gets rejected with `error_kind='session_cost_cap_reached'` and no container starts. With the cap reset to a higher value, subsequent messages run.
