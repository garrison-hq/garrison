...
**M3 — Dashboard read-only.** ✅ Shipped 2026-04-26. Retro: `docs/retros/m3.md`. Next.js 16 + React 19 app reading from Postgres across two roles (`garrison_dashboard_app` for operational reads, `garrison_dashboard_ro` for vault sub-views). Eight surfaces: org overview, department Kanban, ticket detail, hygiene table, vault list/audit/role-secret-matrix, agents registry, operator-invite admin. Activity feed driven by a singleton SSE listener (LISTEN on `work.ticket.created` + parameterised `work.ticket.transitioned.<dept>.<from>.<to>`, plus `event_outbox` poll fallback for finalize/lifecycle events that aren't `pg_notify`'d). better-auth (drizzle adapter, email/password) gates every route; the first-run `/setup` wizard 404s once any user exists; thereafter operators join via atomic invite redemption (`UPDATE … WHERE redeemed_at IS NULL … RETURNING id`, MVCC-serialised). Read-only-first forces you to actually watch the system behave for a few days before giving yourself (and agents) the ability to change state — which is when you catch the "oh, the event payload shape is wrong" class of bugs cheaply. **Notes carried forward from M2 — surfaced, not fixed in M3**: (a) the cost-telemetry blind spot (`agent_instances.total_cost_usd` reads $0.00 on clean finalizes) is now visible in ticket detail with an explicit caveat icon — fix is the supervisor signal-handling change tracked at `docs/issues/cost-telemetry-blind-spot.md`; (b) workspace-sandbox-escape ("artifact claimed vs artifact on disk") is a hygiene-status row in the dashboard's hygiene table — fix lands as Docker-per-agent post-M3, tracked at `docs/issues/agent-workspace-sandboxing.md`. Locked-deps streak deliberately broken on the dashboard side (13 direct + ~150 transitive new TS/JS deps); each justified in the M3 retro under "Dependencies added outside the locked list".

**M4 — Dashboard mutations.** ✅ Shipped 2026-04-27. Retro: `docs/retros/m4.md`. Server-Action mutations for create/edit/transition tickets and edit-agent-config across model / agent.md / listens_for / palace_wing / status. Per-vault-row grants on the dashboard ML side; `secret_metadata` row + `vault_access_log` audit row in the same transaction as every vault write. New `agents.changed` `pg_notify` channel + supervisor-side cache invalidator so config edits propagate to the next spawn without a restart. Operator-invites + theme writes also land here.

**M5 — CEO chat (summoned).** ✅ Shipped 2026-04-28 → 2026-05-01 across four sub-milestones. The arc closes M5; M6 (CEO ticket decomposition + memory hygiene dashboard) starts from this substrate.

**M5.1 — Chat backend (read-only).** ✅ Shipped 2026-04-28. Retro: `docs/retros/m5-1.md`. Per-message ephemeral `garrison-claude:m5` container spawned by the supervisor's chat worker; OAuth token fetched from Infisical at `/operator/CLAUDE_CODE_OAUTH_TOKEN`; SSE producer for turn-by-turn streaming on `/api/sse/chat`; restart + idle sweeps reconcile orphaned `chat_sessions` rows on supervisor boot.

**M5.2 — Chat dashboard surface.** ✅ Shipped 2026-04-29. Retro: `docs/retros/m5-2.md`. Three-pane layout (sidebar / center message stream + composer / right-pane KnowsPane placeholder), multi-session UX with archive + delete affordances, idle-pill chip indicating live session status, sidebar entry between Activity and Hygiene, `/chat/all` full-thread-list view + `/chat/<uuid>` active-session view. New `work.chat.session_deleted` `pg_notify` channel; `chat_messages` FK shifts to `ON DELETE CASCADE` so session delete reaps message rows.

**M5.3 — Chat-driven mutations under autonomous-execution posture.** ✅ Shipped 2026-04-30. Retro: `docs/retros/m5-3.md`. In-tree `internal/garrisonmutate` MCP server exposing a sealed 8-verb set (`create_ticket`, `edit_ticket`, `transition_ticket`, `pause_agent`, `resume_agent`, `spawn_agent`, `edit_agent_config`, `propose_hire`); per-turn tool-call ceiling (default 50, env-tunable) emits an `assistant_error` SSE frame with `bailReason` set when exceeded; tool-use SSE frames flow informatively to the dashboard with no per-call operator gate; concurrency conflicts resolve via `SELECT ... FOR UPDATE NOWAIT` mapped to a typed `ticket_state_changed` error; vault remains opaque to chat per M2.3 Rule 3 carryover. Threat-model amendment landed first at `docs/security/chat-threat-model.md` per the M2.3 vault-threat-model-first pattern.

**M5.4 — "WHAT THE CEO KNOWS" knowledge-base pane.** ✅ Shipped 2026-05-01. Retro: `docs/retros/m5-4.md`. Replaces M5.2's right-pane placeholder with a tabbed `KnowsPane`: Company.md (CEO-editable, MinIO-backed, CodeMirror v6 + lang-markdown editor, ETag-based optimistic concurrency, leak-scan + 64 KB size cap server-side) + Recent palace writes (read-only `mempalace_list_drawers` proxy with refresh + greyed-prior-list-during-fetch UX) + KG recent facts (read-only `mempalace_kg_timeline` proxy with optional source-ticket deep-links). New 4th container (MinIO single-node, digest-pinned, internal-network only, named volume `garrison-minio-data`); new supervisor packages `internal/objstore/` (MinIO wrapper) and `internal/dashboardapi/` (HTTP server on port 8081 with cookie-forward auth against the better-auth `sessions` table); shared `internal/leakscan/` extracted from `internal/finalize`. Two new dependencies: `github.com/minio/minio-go/v7` (Go) and `@uiw/react-codemirror` + `@codemirror/lang-markdown` (TS); both flagged in retro.
...
**M7 — First custom agent end-to-end (hiring + per-agent runtime + immutable preamble).** ✅ Shipped 2026-05-03. Retro: `docs/retros/m7.md`. Threat models: `docs/security/agent-sandbox-threat-model.md`, `docs/security/hiring-threat-model.md`. Three threads merge into one ship (scope expansion 2026-05-02; see `docs/research/m7-spike.md` §4 for the merge rationale):

1. **Hiring flow.** skills.sh integration plus **SkillHub (iflytek)** as the target-state private-skills registry alongside the public skills.sh feed (decision committed 2026-04-24; see `docs/skill-registry-candidates.md`). Proposal UI, approval writes agents + installs skills. Threat model: `docs/security/hiring-threat-model.md`.
2. **Per-agent Docker runtime.** Persistent container per `agents` row, `docker exec` per spawn (10× faster than docker-run; see spike §8 P1). Layered RW workspace + RO root + tmpfs scratch, default-deny networking with opt-in sidecar reach, drop-all-caps + non-root, mandatory cgroup caps. Closes `docs/issues/agent-workspace-sandboxing.md` by structural enforcement. Threat model: `docs/security/agent-sandbox-threat-model.md`.
3. **Immutable prompt-hardening preamble.** Operator-controlled policy text prepended above `agent.md` in every system prompt. Phrased as policy ("Garrison agents: X is prohibited") not identity assertion ("You are Y") to avoid Claude's built-in injection-detection refusal (spike §8 P9).

**M8 — Agent-spawned tickets, cross-department dependencies, runaway control, MCP-server registry.** ✅ Shipped 2026-05-10. Retro: `docs/retros/m8.md`. Closes the event-driven zero-human loop. Four threads ship together (mirrors M7's three-thread merge): (1) any active agent calls `create_ticket` via the M5.3 verb, audited with an `agent_instance_id` anchor (FR-005, FR-401); the agent caller's parent ticket is auto-inherited (FR-006) and cross-dept creates are tagged `cross_dept_create=true` in `args_jsonb` (FR-007). (2) `tickets.depends_on_ticket_id` adds cross-department dependencies, gated at spawn-prep until the predecessor reaches one of its dept's `dependency_satisfaction_columns` (default `["qa_review","done"]`); cycles + 32-hop depth-caps are rejected at `create_ticket` time (FR-103). (3) Per-department `weekly_ticket_budget` runaway gate fires `throttle_events.kind='dept_weekly_ticket_budget_exceeded'` on the same write path as `create_ticket`; the rejection bookkeeping commits in a fresh tx so the main-flow rollback doesn't erase it. NULL budget = unlimited (M8 alpha default per FR-200). (4) **MCPJungle** (self-hosted Go service, Postgres-backed, MPL-2.0, enterprise-mode ACLs) runs as a compose sidecar; per-agent McpClient names follow `<customer_slug>.<role-slug>.<short-agent-id>` per FR-304/FR-307; bearer tokens land at vault path `mcpjungle/agents/<agent-id>` via the M8 reconciler (called from `cmd/supervisor/main.go` after vault init); the reactive `mcpserverwork.Worker` consumes `pg_notify('work.mcp_server.registration_requested')` from the M8 trigger and uses row-level `SELECT FOR UPDATE NOWAIT` for LISTEN+poll dedupe. Server-Action-only `register_mcp_server` verb lives in `internal/garrisonmutate/server_action_verbs.go` (disjoint from chat-side `Verbs` per the M7 F3 lean pattern). Single-instance for M8 alpha; per-customer MCPJungle instance is the committed beta path (Option A — hard network/DB isolation, per-customer admin tokens in vault), with the `Client.URLForCustomer` method as the structural seam. M7-kickoff maturity re-check fired positive 2026-05-03 (`docs/research/m8-mcpjungle-spike.md`). See `docs/mcp-registry-candidates.md` + `specs/_context/m8-context.md`.
...
- **`ticket` mode** — inserts into `tickets` and flows through the department's normal Kanban workflow. Used for work with real handoffs (research-and-brief, build-and-ship). Exits through M2.2.1's `finalize_ticket`.
- **`oneshot` mode** — spawns the agent directly with a bounded brief; no ticket row, no column transitions, no Kanban surface. Used for cadence reflections (daily team standup, weekly drift-check, metric report — "wake up, read state, write findings, exit"). Surfaces in a separate "Recurring jobs" dashboard view. Exits through a new sibling MCP tool, **`finalize_oneshot`** — parallel to `finalize_ticket` but accepts no `ticket_id` and commits no column transition; structured outcome, hygiene-write verification, and `agent_instances` audit row are unchanged.

Both modes inherit M6 cost-throttle at spawn, M8 runaway control, hygiene-write verification on exit, and the `agent_instances` audit row. Only the ticket row and column-hop machinery is skipped in `oneshot` mode.
...
**M10 — Ingress connectors (the company becomes reactive to the outside world).** ✅ Shipped 2026-06-12. Retro: `docs/retros/m10.md`. Today the event bus is internal-only: every ticket originates from an operator, an agent (M8), or a schedule (M9). Real companies are reactive organisms — most work _arrives_. M10 adds inbound connectors that normalize external events into tickets on the existing bus: email-in, Stripe webhooks (payment failed, subscription created/cancelled), and GitHub (issues, mentions, review requests). Design commitments: connectors are supervisor-side components, never agent-reachable; each external event carries an idempotency key (webhook redelivery and email re-threading must not double-create tickets — same discipline as M1's `processed_at` and M9's `FOR UPDATE SKIP LOCKED`); every created ticket is tagged with connector provenance in `tickets.metadata` JSONB (keys `ingress_connector`, `external_id`, `external_url`) so downstream hygiene can distinguish "operator asked" from "the internet asked." Noise filtering happens at the connector (spam, bot traffic, duplicate GitHub notifications), not in agent prompts — the ticket stream stays clean by construction; this commitment covers _mechanical_ noise only, not semantic classification, which a deterministic connector cannot do. Per-connector rate caps feed the M6 throttle machinery so a webhook storm cannot stampede spawns. Threat model: `docs/security/ingress-threat-model.md` (committed before any connector code, FR-800).

Routing is two-tier, because external events split on ambiguity. **Structured events route directly**: a Stripe `payment_failed` is unambiguous and the connector inserts it straight into the owning department's intake column — no LLM in the webhook path, no triage latency. **Ambiguous events land in a triage inbox**: email-in and GitHub mentions can be technical (engineering), commercial (a partnership inquiry), or anything between, so the connector's only routing decision is "this needs triage" — the ticket lands in a triage column where a Concierge-style triage agent listens. This reuses the existing triage-by-agent pattern (the CTO on engineering's `todo` is the within-department version) at company level: the triage agent reads the ticket, classifies semantically, and routes via M8's `create_ticket` cross-department (tagged `cross_dept_create=true`, parent auto-inherited, audited), then closes the inbox ticket — provenance chain intact. Connectors stay dumb and deterministic; routing intelligence lives where garrison puts all intelligence, in an agent with a ticket in front of it. Whether the triage inbox is a standing "front desk" department or a per-connector column is a spec decision, as is the direct-route rule table per connector.

The likely first connector is GitHub, because the first dogfood workload (sortie issue triage → Concierge) needs it — and that workload is exactly the triage-inbox path above.

Shipped implementation pointers, per thread: (1) **Connector framework + GitHub connector** — `supervisor/internal/ingress/` (`connector.go` interface + `FilterDecision` enum + sentinel errors; `signature.go` HMAC-SHA256 raw-body verification over `X-Hub-Signature-256`; `idempotency.go` `insertDelivery` with `23505`-as-dedup-signal; `handler.go` SR6 pipeline with `io.LimitReader` 26 MB guard + `sync/atomic` rejection counter; `github.go` `GitHubConnector` Filter/Map + `render.go` bounded `strings.ReplaceAll` template); `supervisor/internal/throttle/ingress.go` `FireIngressRateCap` writing `throttle_events.kind='ingress_rate_cap_exceeded'`; `supervisor/internal/ingress/ratecap.go` in-process token-bucket per connector. (2) **Ingress server lifecycle** — `supervisor/internal/ingress/server.go` mirroring `dashboardapi.Server`; dedicated port 8082 in the existing errgroup; vault-fetched webhook secret at boot (fail-closed per FR-302); wired from `supervisor/cmd/supervisor/main.go`. (3) **Dashboard** — ingress-origin chip on `TicketCard`; external-source link in ticket detail; connector-status surface at `/admin/connectors`; `GET /ingress/status` on dashboardapi port 8081 for per-connector bad-signature rejection counts. Schema: `ingress_deliveries` table + `throttle_events.kind` extension — migration `20260612000000_m10_ingress_connectors.sql`. Zero new Go or TypeScript dependencies.

**M11 — Action Broker (outbound external actions, gated by policy).** ✅ Shipped 2026-06-12. Retro: `docs/retros/m11.md`. The keystone of the AI-native-company arc. Agents can currently _think_ and _write internally_; M11 is how they _act on the world_ without ever holding the ability to do so directly. Shape: a sealed `request_external_action` MCP verb (joining the M5.3/M8/M9 sealed-registry discipline) writes a pending-action row; a **tier policy table** classifies the action — `auto` (act + log), `notify` (act, then tell the operator), `approve` (blocked until operator click), `human_only` (agent prepares, dispatcher never executes); a dispatcher worker executes approved/auto actions using vault-scoped credentials the agent containers never see; an **Outbox** dashboard surface is the operator's approval queue. This generalizes the M7 hiring-queue pattern (propose → human approves → system executes) into the universal outbound path. Two doctrines carry over hard: policy lives in the tier table, not in prompts (M2.2.1 mechanism-over-prompt), and M7's default-deny container networking is the enforcement primitive — agents _physically cannot_ reach external services, so the broker is the only door, not a convention. Anything public-facing (posts, replies, releases, mail to non-customers, price/copy changes) is Approve-tier **permanently**: reputation damage is the existential risk for an agent-operated company, and one spammy autopost costs more than a year of approval clicks saves. Every action row is immutable and anchored to its `agent_instance_id`, same as the M8 audit posture. Threat model: `docs/security/action-broker-threat-model.md` (committed before any dispatcher code, FR-030/SC-008).

Shipped implementation pointers, per thread: (1) **12th sealed verb** — `garrisonmutate/verbs_actions.go` (`RequestExternalActionArgs`, six-step handler: agent-caller guard → parse/validate → `Classify` → one-tx InsertPendingAction + outcome + audit → dispatch-notify for auto/notify → typed Result with `AffectedResourceURL: "/admin/outbox"`); `garrisonmutate/verbs.go` entry 12; `garrisonmutate/tool.go` `agentVerbNames()` extension; three new `ServerActionVerbs` entries (`approve_action`, `reject_action`, `mark_action_done` RC1) in `garrisonmutate/server_action_verbs.go`. (2) **Tier policy + permanent-Approve floor** — `supervisor/internal/actionbroker/policy.go` (`Tier` type + four constants; `floor` map consulted before `policy` map; `Classify(actionType) (Tier, string)` — floor wins over any policy override; unknown types default to `approve`; `CONSTRAINT pending_actions_floor_is_approve` DB CHECK mirrors the floor set exactly — no drift). (3) **Dispatcher worker** — `supervisor/internal/actionbroker/dispatcher.go` (`RunLoop` poll-fallback ticker; `Worker.Handle` per-row state machine: claim via `FOR UPDATE SKIP LOCKED` → tier-gated dispatchability → vault-fetch fail-closed → `GitHub.PostComment` → terminal status transition + immutable outcome); `supervisor/internal/actionbroker/github.go` (`PostComment` via stdlib `net/http`; PAT sent as `Authorization: token <PAT>`, never logged; `ErrRecoverable` for `429`/`5xx`, terminal failure for `404`/`422`; `SecretValue.Zero()` after use); wired into `cmd/supervisor/main.go` errgroup alongside the M9 scheduler and M10 ingress server. (4) **Outbox dashboard surface** — `dashboard/app/[locale]/(app)/admin/outbox/` (Server Component listing `approve`-tier pending rows; approve/reject/mark-as-done Server Actions in `actions.ts`; sidebar link under admin group); `dashboard/lib/queries/outbox.ts` (drizzle-direct reads). Schema: `work.pending_actions` + `work.pending_action_outcomes` + `chat_mutation_audit` CHECK extensions — migration `20260612000001_m11_action_broker.sql`. Zero new Go or TypeScript dependencies (GitHub comment-create via stdlib `net/http`; FR-020/SC-009).
...
**Repo layout:** see [`docs/agents/repository-layout.md`](docs/agents/repository-layout.md) for the canonical annotated tree (current through M5.4). The README also carries a top-level summary.
