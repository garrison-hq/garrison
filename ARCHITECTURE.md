...
**M8 — Agent-spawned tickets, cross-department dependencies, runaway control, MCP-server registry.** Closes the event-driven zero-human loop. Four threads ship together (mirrors M7's three-thread merge): (1) any active agent calls `create_ticket` via the M5.3 verb, audited with an `agent_instance_id` anchor; (2) `tickets.depends_on_ticket_id` adds cross-department dependencies, gated at spawn-prep until the dependency reaches a configured satisfaction state; (3) per-department weekly ticket-creation budget on the same write path as `create_ticket` (extends M6's `throttle_events` with a new kind); (4) **MCPJungle** chosen as the MCP-server registry/proxy (self-hosted, Go-based, Postgres-backed, MPL-2.0, enterprise-mode ACLs). Single-instance for M8 alpha; per-customer MCPJungle instance is the committed beta path (Option A — hard network/DB isolation, per-customer admin tokens in vault). M7-kickoff maturity re-check fired positive 2026-05-03 (`docs/research/m8-mcpjungle-spike.md`). See `docs/mcp-registry-candidates.md` + `specs/_context/m8-context.md`.
